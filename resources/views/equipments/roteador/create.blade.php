        ...
        <div class="container-fixed flex items-center justify-between flex-wrap gap-3">
            <div class="flex items-center flex-wrap gap-1 lg:gap-5">
                ...
                <div class="flex items-center flex-wrap gap-1 text-sm">
                    ...
                    <span class="text-gray-700">Dispositivos</span>
                    <span class="text-gray-400 text-sm">/</span>
                    <span class="text-gray-900">Adicionar Roteador</span>
                </div>
            </div>
        </div>
    </div>
            ...
            @endif
                ...
                <div class="card pb-2.5">
                    <div class="card-header" id="basic_settings">
                        <h3 class="card-title">Informações Roteador</h3>
                    </div>
                    ...
                    <div class="card-body grid gap-5">
                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            <label class="form-label max-w-56">Cliente</label>
                ...
                <div class="card pb-2.5">
                    <div class="card-header" id="wifi_settings">
                        <h3 class="card-title">Rede Wi-Fi</h3>
                    </div>

                    <div class="card-body grid gap-5">
                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            <label class="form-label max-w-56">SSID</label>
                            @if(isset($edit))
                                <input class="input" type="text" placeholder="Nome da rede Wi-Fi" value="{{ $equipment->wifi->ssid ?? '' }}" name="wifi_ssid" />
                            @else
                                <input class="input" type="text" placeholder="Nome da rede Wi-Fi" value="{{ old('wifi_ssid') }}" name="wifi_ssid" />
                            @endif
                            @error('wifi_ssid') <div class="text-danger">{{ $message }}</div> @enderror
                        </div>

                        <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
                            <label class="form-label max-w-56">Senha Wi-Fi</label>
                            @if(isset($edit))
                                <input class="input" type="text" placeholder="Senha da rede Wi-Fi" value="{{ $equipment->wifi->password ?? '' }}" name="wifi_password" />
                            @else
                                <input class="input" type="text" placeholder="Senha da rede Wi-Fi" value="{{ old('wifi_password') }}" name="wifi_password" />
                            @endif
                            @error('wifi_password') <div class="text-danger">{{ $message }}</div> @enderror
                        </div>
                    </div>
                </div>
        ...
        <div class="container-fixed">
            <div class="flex flex-col md:flex-row justify-center md:justify-between items-center gap-3 py-5">
                <div class="flex order-2 md:order-1 gap-2 font-normal text-2sm">
                    <span class="text-gray-500">{{ date_format(date_create(), 'Y') }}©</span>
...
<script defer src="{{ asset('js/helper-phone.js') }}"></script>
<script defer src="{{ asset('js/helper.js') }}"></script>
<script defer src="{{ asset('js/addUserEquip.js') }}"></script>
